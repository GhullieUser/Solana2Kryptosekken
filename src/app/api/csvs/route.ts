	...
	if (id || address) {
		if (address && format === "list") {
			const { data, error } = await supabase
				.from("generated_csvs")
				.select(
					"id,address,label,created_at,updated_at,raw_count,processed_count,from_iso,to_iso,include_nft,use_oslo,dust_mode,dust_threshold,dust_interval"
				)
				.eq("address", address)
		...
		const { data, error } = await supabase
			.from("generated_csvs")
			.select(
				"csv_text,address,label,raw_count,processed_count,from_iso,to_iso,include_nft,use_oslo,dust_mode,dust_threshold,dust_interval,created_at,updated_at"
			)
